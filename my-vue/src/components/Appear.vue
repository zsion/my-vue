<template>
  <div>
    ...
    <mt-header fixed title="中国上市" style="position:fixed;top:0;left:0;">
...
export default {
  ...
  methods: {
    ...
    changeList1:function(){
      ...
      this.styles = "0"
      this.last = 0
      this.$http.post(this.GetList,{keywords:this.$refs.searchkey.value,type:0,page:this.last,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
          if(response.body.Data.length == 0){
            this.list_data = [];
            this.iscondition = false
    ...
    changeList2:function(){
      ...
      this.styles = "1"
      this.last = 0
      this.$http.post(this.GetList,{keywords:this.$refs.searchkey.value,type:1,page:this.last,token:getCookie("token")},{emulateJSON: true}).then(response => {
        //console.log(response.body.Data)
        if(response.body.ResultType == 1){
          if(response.body.Data.length == 0){
            this.list_data = [];
...
.tabhover a{border-bottom: 2px solid #00BEEE;color:#0F0F0F;padding:0 1.5rem .6rem 1.5rem;}
