    ...
    <div class="detail_con">
      <ul>
        <li>
          ...
          <p><span>序号：</span><span>{{detaildata.xuhao}}</span></p>
          <p><span>原料药（中文）：</span><span>{{detaildata.yly}}</span></p>
        </li>
        <li>
          <p><span>商品名：</span><span>{{detaildata.spm}}</span></p>
          <p><span>厂商国家（中文）：</span><span>{{detaildata.scqy}}</span></p>
          <p><span>剂型细分：</span><span>{{detaildata.jixing}}</span></p>
          <p><span>规格：</span><span>{{detaildata.guige}}</span></p>
        </li>
        <li>
          <p><span>注册证号：</span><span>{{detaildata.zczh}}</span></p>
          <p><span>生产厂商（英文）：</span><span>{{detaildata.scqyyw}}</span></p>
          <p><span>持证商：</span><span>{{detaildata.czgs}}</span></p>
          <p><span>参比制剂来源：</span><span>{{detaildata.laiyuan}}</span></p>
        </li>
        <li>
          <p><span>产地：</span><span>{{detaildata.chandi}}</span></p>
          <p><span>备注2：</span><span>{{detaildata.beizhu2}}</span></p>
          <p><span>发布时间：</span><span>{{detaildata.fbsj}}</span></p>
          <p><span>通告名：</span><span>{{detaildata.tgm}}</span></p>
        </li>
        <li>
...
export default {
  ...
  data () {
    return {
      backbtn:true,
      GetInfo:ApiConfig.Yzxpj.GetInfo,
  ...
  methods: {
    ...
    pubgetdata:function(){
      ...
      this.$http.post(this.GetInfo,{xuhao:pzwh,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
