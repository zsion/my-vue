      ...
      <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore">
        <ul>
          <li v-for="usedata in list_data">
            <h3>{{usedata.ypmc}}</h3>
            <p><span>承办日期：</span><span>{{usedata.cbrq}}</span></p>
            <p><span>申报企业：</span><span>{{usedata.scqy}}</span></p>
            <p><span>受理号：</span><span>{{usedata.slh}}</span><router-link :to="{ name: 'ZgsbDetail', query: {pzwh:usedata.slh}}">详情</router-link></p>
            <p><span>办理状态：</span><span>{{usedata.blzt}}</span></p>
          </li>
        </ul>
      </mt-loadmore>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      iscondition:true,
      searchkeycon:"",
      GetList:ApiConfig.Zgsb.GetList,
  ...
  methods: {
    ...
    more:function(){
      Indicator.open('加载中...');
      this.last++
      this.$http.post(this.GetList,{keywords:this.search_Con,page:this.last,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
          if(response.body.Data.length > 0){
