<template>
  <div>
    ...
    <div class="fixeds">
      <div class="part-two">
        <ul>
          <li v-bind:class="{ tabhover: isActiveone }" @click="changeList1()"><a href="javascript:void(0)" >国产药品</a></li>
          <li><b>|</b></li>
          <li v-bind:class="{ tabhover: isActivetwo }" @click="changeList2()"><a href="javascript:void(0)" >进口药品</a></li>
        </ul>
      </div>
    </div>
      ...
      <mt-loadmore :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" :auto-fill="false" ref="loadmore">
        <ul>
          <li v-for="usedata in list_data">
            <h3>{{usedata.ypmc}}</h3>
            <p><span>规格：</span><span>{{usedata.guige}}</span></p>
            <p><span>持证商：</span><span>{{usedata.czgs}}</span></p>
            <p><span>治疗小类：</span><span>{{usedata.zlxl}}</span></p>
            <p v-if="styles == 0"><span>批准日期：</span><span>{{usedata.pzrq}}</span></p>
            <p v-else-if="styles == 1"><span>发证日期：</span><span>{{usedata.fzrq}}</span></p>
            <p v-if="styles == 0"><span>批准文号：</span><span>{{usedata.pzwh}}</span><router-link :to="{ name: 'Detail', query: { styles:styles,pzwh:usedata.pzwh}}">详情</router-link></p>
            <p v-else-if="styles == 1"><span>注册证号：</span><span>{{usedata.zczh}}</span><router-link :to="{ name: 'Detail', query: { styles:styles,zczh:usedata.zczh}}">详情</router-link></p>
          </li>
        </ul>
      </mt-loadmore>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      iscondition:true,
      searchkeycon:"",
      GetList:ApiConfig.Zgss.GetList,
  ...
  methods: {
    ...
    pubgetdata:function(){
      ...
      this.$http.post(this.GetList,{keywords:sned_Con,type:0,page:0,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
          ...
          //this.list_data = response.body.Data.length > 0 ? response.body.Data : []
          Indicator.close();
        }else{
          Indicator.close();
          this.$toast(response.body.Message)
    ...
    changeList1:function(){
      ...
      this.styles = "0"
      this.last = 0
      this.$http.post(this.GetList,{keywords:this.search_Con,type:0,page:this.last,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
          if(response.body.Data.length == 0){
            this.list_data = [];
            this.iscondition = false
    ...
    changeList2:function(){
      ...
      this.styles = "1"
      this.last = 0
      this.$http.post(this.GetList,{keywords:this.search_Con,type:1,page:this.last,token:getCookie("token")},{emulateJSON: true}).then(response => {
        if(response.body.ResultType == 1){
          if(response.body.Data.length == 0){
            this.list_data = [];
            this.iscondition = false
...
.read_title p{height:2.5rem;line-height:2.5rem;color: #31363B;font-size: 1rem; -webkit-margin-before: 0;-webkit-margin-after: 0;text-align: left;padding-left: 1rem;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
...
.fixeds{position: fixed;top:5rem;left:0;width:100%;z-index: 2;}
...
.part-one ul{width:90%;margin:0 auto;padding:0 0 .5rem;-webkit-padding-start: 0;-webkit-padding-end: 0;-webkit-margin-before: 0;-webkit-margin-after: 0;display: flex;flex-direction: row;align-items: center;justify-content: space-around;}
.part-one ul li{list-style-type: none;}
.part-one ul li:nth-child(1){width:70%;}
.part-one ul li:nth-child(2){width:20%;}
.part-one ul li input{width:100%;font-size: .875rem;background: #F2F3F5;border-radius: 42px;border: none;padding: .625rem .625rem .625rem;float: left;outline: none;}
.part-one ul li button{width:100%;border:none;background:transparent;border-radius:100%;padding-top: .25rem;outline: none;}
.part-one ul li button img{width:80%;}

.part-two{background: #fff;margin-bottom: .625rem;max-width: 768px;margin: auto;}
